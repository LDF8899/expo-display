...
function renderDisplayNav(metaText) {
  if (!displayNav) return;
  const fallback = ["社会服务", "国际交流", "育人成果", "名师名匠"];
  const items = String(metaText || "")
    .split(/\s*[·•|｜/、,，]\s*/)
    .map((item) => item.trim())
    .filter(Boolean)
    .slice(0, 4);
  const navItems = items.length >= 2 ? items : fallback;
  displayNav.innerHTML = "";
  navItems.forEach((item, index) => {
    const pill = document.createElement("span");
    pill.className = "nav-pill";
    pill.dataset.odId = `nav-portal-${index + 1}`;
    pill.textContent = item;
    displayNav.appendChild(pill);
  });
}
...
function applyProject(project) {
  ...
  renderDisplayNav(schoolMetaText);
