...
(function () {
  ...
  var MODULE_META = {
    ...
    media: ["数字资源", "专题视频、资源库、课程资源、扫码资料和数字素材。"],
    systems: ["特色系统入口", "对接系部特色系统、业务平台或互动入口。"],
    resources: ["数字资源", "集中呈现数字资源、专题内容和可扫码访问资料。"]
  };
  var MODULE_SEQUENCE = [
    ["overview", "概览", "基本情况"],
    ["majors", "专业", "专业设置"],
  ...
  var ACHIEVEMENT_MARKET_CATEGORIES = [
    { key: "masters", label: "名匠名师", color: "#7030A0", sectionId: "masters", description: "教学名师、技能大师、双师团队和大师工作室。" },
    { key: "alumni", label: "优秀校友", color: "#0070C0", sectionId: "alumni", description: "校友人物、成长经历、就业典型和行业贡献。" },
    { key: "students", label: "优秀学生", color: "#30C0B4", sectionId: "students", description: "学生风采、技能成长、竞赛经历和榜样故事。" },
    { key: "teaching", label: "教学科研", color: "#75BD42", sectionId: "achievements", description: "教学成果、科研项目、实践案例和建设成效。" },
    { key: "competitions", label: "技能大赛", color: "#EFBB1F", sectionId: "competitions", description: "赛事、获奖、承办活动和比赛现场。" },
    { key: "honors", label: "荣誉资质", color: "#FB9236", sectionId: "honors", description: "证书、奖项、资质认定和荣誉成果。" }
  ];
  ...
  function inlineEditApi(url, options) {
    ...
    return fetch(url, Object.assign({}, options, { headers: headers, signal: options.signal || controller.signal }))
      ...
      .catch(function (error) {
        clearTimeout(timeout);
        if (error.name === "AbortError") throw new Error("鉴权或保存请求超时，请确认本地服务正常运行后重试");
        throw error;
      });
  }
    ...
    HOME_CARDS = INLINE_EDIT.projects
      ...
      .map(function (project) {
        ...
        return {
          ...
          label: cfg.portalHomeCardLabel || "系部专题",
  ...
  function resetInlineDefaults() {
    ...
    INLINE_EDIT.projects.forEach(function (project) {
      ...
      if (project.portalType === "school") {
        ...
        cfg.portalHomeCopy = "以系部专题为主线，串联专业建设、实训基地、产教融合、教学成果与专题资源。";
        cfg.portalHomeRouteLabels = ["学校门户", "系部专题", "板块资料", "专题展区"];
  ...
  function createInlineToolbar() {
    ...
    var adminHref = INLINE_EDIT.canReview ? "/admin" : "/teacher?view=pages";
    var adminLabel = INLINE_EDIT.canReview ? "后台管理" : "我的任务";
    toolbar.innerHTML = '<strong>预览编辑</strong><span data-inline-edit-status>点文字改、点图换</span>' +
      ...
      '<a class="inline-edit-admin-link" href="' + adminHref + '">' + adminLabel + '</a>' +
  ...
  function saveInlineEdits() {
    var ids = inlineCanEditProjectConfig() ? Object.keys(INLINE_EDIT.dirty) : [];
  ...
  function inlineAssignedSectionKeys() {
    return (INLINE_EDIT.assignmentScope || []).map(function (item) {
      return String((item && (item.key || item.moduleKey)) || "").trim();
    }).filter(Boolean);
  }

  function inlineCanEditProjectConfig() {
    return !!INLINE_EDIT.canReview;
  }

  function inlineCanEditSection(sectionId) {
    if (INLINE_EDIT.canReview) return true;
    var keys = inlineAssignedSectionKeys();
    if (!keys.length) return false;
    return keys.indexOf(String(sectionId || "").trim()) >= 0;
  }

  function addInlineNoPermissionBadge(container, sectionId) {
    if (!INLINE_EDIT.enabled || INLINE_EDIT.canReview || inlineCanEditSection(sectionId)) return;
    if (!container || container.querySelector(".inline-edit-no-permission")) return;
    container.classList.add("inline-edit-readonly-section");
    var badge = document.createElement("span");
    badge.className = "inline-edit-no-permission inline-edit-control";
    badge.textContent = "未分配权限";
    badge.title = "当前老师账号未分配该板块，只能预览，不能编辑。";
    container.appendChild(badge);
  }

  function inlineNoPermissionBadgeHtml(sectionId) {
    if (!INLINE_EDIT.enabled || INLINE_EDIT.canReview || inlineCanEditSection(sectionId)) return "";
    return '<span class="inline-edit-no-permission inline-edit-control" title="当前老师账号未分配该板块，只能预览，不能编辑。">未分配权限</span>';
  }

  function systemsSectionFor(data) {
    var sections = (data && data.sections) || [];
    for (var i = 0; i < sections.length; i += 1) {
      if (sections[i] && sections[i].id === "systems") return sections[i];
    }
    var section = {
      id: "systems",
      title: "特色系统入口",
      blocks: [{ type: "text", content: "维护合作商系统、互动体验平台或外部专题入口。" }],
      contentType: "article",
      isTemplateSlot: true
    };
  ...
  function systemsContentItemFor(data) {
    var items = (data && data.sectionItems && data.sectionItems.systems) || [];
    if (items.length) {
      var primary = items[0];
      primary.extraItemIds = items.slice(1).map(function (item) { return item.id; }).filter(Boolean);
      return primary;
    }
    var sections = (data && data.sections) || [];
    for (var i = 0; i < sections.length; i += 1) {
      var section = sections[i];
      if (section && section.id === "systems" && Array.isArray(section.externalLinks) && section.externalLinks.length) {
        return {
          id: section.contentItemId || "",
          title: section.title || "特色系统入口",
          summary: section.blocks && section.blocks[0] ? section.blocks[0].content || "" : "",
  ...
  function applyInlineDetailControls() {
    ...
    if (state.kind === "departments" && inlineCanEditProjectConfig()) {
    ...
    if (state.kind === "departments") {
      document.querySelectorAll(".showcase-card[data-module]").forEach(function (card) {
        ...
        if (!inlineCanEditSection(moduleId)) {
          addInlineNoPermissionBadge(wrap || card, moduleId);
          return;
        }
      ...
      var canEditCurrentSection = inlineCanEditSection(section && section.id);
      var heading = document.querySelector(".topic-section-heading");
      if (heading && canEditCurrentSection) {
        bindInlineSectionTitle(heading.querySelector("h2"), section, project);
        if (inlineCanEditProjectConfig()) addInlineDeleteSectionButton(heading, project, section);
      }
      ...
      if (canEditCurrentSection && leadNode && model.leadTextBlock) bindInlineBlockText(leadNode, project, section, [model.leadTextBlock], 0);
      var storyTitle = document.querySelector(".topic-section-story h3");
      if (canEditCurrentSection && storyTitle && model.titleTextBlock) bindInlineBlockText(storyTitle, project, section, [model.titleTextBlock], 0);
      ...
      if (canEditCurrentSection && addPoint && !addPoint.dataset.inlineBound) {
      ...
      if (mediaBox) addInlineNoPermissionBadge(mediaBox, section.id);
      if (canEditCurrentSection && mediaBox) addInlineMediaImageButton(mediaBox, project, section);
      document.querySelectorAll("[data-upload-video]").forEach(function (btn) {
        if (!inlineCanEditSection("media")) {
          addInlineNoPermissionBadge(btn.parentElement || btn, "media");
          btn.hidden = true;
          return;
        }
        if (btn.dataset.inlineBound) return;
      ...
      if (coopPhoto || coopTitle || coopBody) {
        var canEditCoop = inlineCanEditSection("cooperation");
        ...
        if (canEditCoop && coopPhoto) addInlineMediaImageButton(coopPhoto, project, coopSection);
        if (canEditCoop && coopTitle) bindInlineSectionTitle(coopTitle, coopSection, project);
        if (canEditCoop && coopBody) bindInlineCoopBody(coopBody, project, coopSection);
      }
      document.querySelectorAll("[data-coverflow-edit]").forEach(function (btn) {
        if (!inlineCanEditSection(btn.dataset.coverflowEdit)) {
          addInlineNoPermissionBadge(btn.parentElement || btn, btn.dataset.coverflowEdit);
          btn.hidden = true;
          return;
        }
        if (btn.dataset.inlineBound) return;
      ...
      document.querySelectorAll(".topic-showcase-mini[data-section-id], .topic-showcase-partner[data-section-id]").forEach(function (card) {
        if (!inlineCanEditSection(card.dataset.sectionId)) {
          addInlineNoPermissionBadge(card, card.dataset.sectionId);
          return;
        }
        ...
        if (inlineCanEditProjectConfig()) addInlineDeleteSectionButton(card, project, sec);
      });
    }
    if (inlineCanEditProjectConfig()) attachDetailAddSection(data, project);
      ...
      fileToDataUrl(file)
        ...
        .then(function (payload) {
          ...
          if (vb) {
            ...
            blocks.push({ type: "video", src: url, poster: poster, title: "数字资源" });
          }
          section.blocks = blocks;
          markPageDirty(project.id, section.id, pagePayloadForSection(section));
          renderCurrentDetail();
  ...
  function initInlineEdit() {
    ...
    INLINE_EDIT.initPromise = inlineEditApi("/api/session")
      .then(function (session) {
        ...
        if (!session.authenticated || !permissions.some(function (item) { return item === "admin" || item === "review" || item === "department_admin" || item === "teacher"; })) {
          var next = location.pathname + location.search + location.hash;
          location.href = "/login?next=" + encodeURIComponent(next);
          throw new Error("请先登录后台账号，登录后会自动回到可视化编辑页面。");
        }
        INLINE_EDIT.csrfToken = session.csrfToken || "";
        INLINE_EDIT.role = permissions.indexOf("teacher") >= 0 ? "teacher" : "review";
        INLINE_EDIT.canReview = permissions.some(function (item) { return item === "admin" || item === "review" || item === "department_admin"; });
      ...
      .then(function (payload) {
        ...
        if (!INLINE_EDIT.canReview && !screenDetail.hidden) {
          var data = currentDetailData();
          var project = detailProjectFor(data);
          if (project && project.id) {
            return inlineEditApi("/api/projects/" + encodeURIComponent(project.id) + "/assignments")
              .then(function (assignmentsPayload) {
                INLINE_EDIT.assignmentScope = assignmentsPayload.assignmentScope || [];
  ...
  function mergePortalPayload(kind, id, payload) {
    ...
    data.achievementMarket = payload.achievementMarket || null;
  ...
  function externalLinksForContentItem(item) {
    ...
    return assets.filter(function (asset) {
      ...
      return {
        label: asset.caption || asset.title || item.title || "打开特色系统",
  ...
  function sectionIdForPage(kind, page, index) {
    ...
    if (kind === "departments") {
      ...
      if (moduleKey === "systems" || moduleKey === "resources") return moduleKey;
  ...
  function displaySectionTitle(section) {
    if (state.kind === "topics") {
      if (section && section.id === "media") return "数字资源";
      return TOPIC_SECTION_TITLES[section.id] || section.title;
    }
  ...
  function moduleTextFor(data, moduleId) {
    ...
    if (moduleId === "media") return "可接入专题视频、资源库、课程资源、扫码资料或数字素材。";
    if (moduleId === "systems") return "特色系统入口用于承接系部业务平台、训练系统、互动系统或外部专题入口。";
    if (moduleId === "resources") return "数字资源用于聚合专题内容、数字素材、课程资源和扫码访问资料。";
    return "";
  }
  ...
  function renderDepartmentShowcase(data) {
    ...
    return '<section class="showcase-board" aria-label="' + esc(data.name) + '展示页面">' +
      ...
      '<h2>' + esc(data.name) + '</h2><p>' + esc(data.summary || DEPT_SUMMARY[data.id] || "系部专题") + '</p></div>' +
      ...
      galleryHtml + '</article>' +
      '<article class="showcase-panel video-panel"><div class="showcase-panel-head"><h3>数字资源</h3><span>播放区</span></div>' +
      videoHtml + '</article>' +
      '</div></section>';
  }
  ...
  function renderTopicCoverflowCarousel(items, options) {
    options = options || {};
      ...
      var editSectionId = item.editSectionId || item.sectionId || "";
      var editBtn = INLINE_EDIT.enabled && editSectionId
        ? '<button type="button" class="inline-edit-media-button inline-edit-control" data-coverflow-edit="' + esc(editSectionId) + '" aria-label="更换图片">换图</button>'
        : "";
      var manageBtn = INLINE_EDIT.enabled && item.manageUrl
        ? '<a class="inline-edit-market-button inline-edit-control" href="' + esc(item.manageUrl) + '">前台编辑</a>'
        : "";
      return '<article class="topic-coverflow-slide' + (index === 0 ? " is-active" : "") + (item.video ? " is-video" : "") + '"' +
        ' style="--slide-image: ' + coverflowImageUrl(src) + '; --market-color:' + esc(item.color || "rgba(255,177,26,0.82)") + '"' +
        (item.sectionId ? ' data-section-id="' + esc(item.sectionId) + '"' : "") +
        (item.marketUrl ? ' data-market-url="' + esc(item.marketUrl) + '"' : "") +
    ...
    root.querySelectorAll("[data-topic-coverflow]").forEach(function (carousel) {
      ...
      slides.forEach(function (slide, idx) {
        slide.addEventListener("click", function (e) {
          e.stopPropagation();
          if (e.target.closest(".inline-edit-control")) return;
          if (slide.dataset.marketUrl && idx === active) {
            location.href = slide.dataset.marketUrl;
            return;
          }
  ...
  function topicMarketCategoryItems(data, key) {
    var market = data && data.achievementMarket;
    var categories = (market && market.categories) || [];
    var category = categories.find(function (item) { return item && item.key === key; });
    return (category && category.items) || [];
  }

  function topicMarketCategoryCover(data, category, items) {
    var sectionImage = sectionImages(data, category.sectionId)[0] || null;
    if (sectionImage && sectionImage.src) return {
      imageUrl: sectionImage.src,
      caption: sectionImage.caption || category.label
    };
    var itemWithImage = (items || []).find(function (item) { return item && item.imageUrl; }) || null;
    if (itemWithImage) return {
      imageUrl: itemWithImage.imageUrl,
      caption: itemWithImage.title || category.label
    };
    return {
      imageUrl: data.cover || "/static/blueprint/" + (data.id || "digital-tourism") + "-bg.png",
      caption: category.label
    };
  }

  function renderTopicAchievementMarketCarousel(data) {
    if (!data || data.id === "campus-culture" || !data.achievementMarket) return "";
    var slides = ACHIEVEMENT_MARKET_CATEGORIES.map(function (category) {
      var items = topicMarketCategoryItems(data, category.key);
      var cover = topicMarketCategoryCover(data, category, items);
      var countText = items.length ? items.length + " 条成果" : "待补充成果";
      return {
        title: category.label,
        body: countText + " · " + category.description,
        imageUrl: cover.imageUrl,
        caption: "成果超市入口 · " + countText,
        color: category.color,
        editSectionId: category.sectionId,
        marketUrl: "/display?market=" + encodeURIComponent(category.key) + "&topic=" + encodeURIComponent(data.id || ""),
        manageUrl: "/display?market=" + encodeURIComponent(category.key) + "&topic=" + encodeURIComponent(data.id || "") + "&edit=1"
      };
    });
    return renderTopicCoverflowCarousel(slides, {
      fallbackImage: data.cover || "/static/blueprint/" + (data.id || "digital-tourism") + "-bg.png",
      title: "教学与创新成果"
    });
  }
  ...
  function renderTopicShowcaseOverview(data) {
    ...
    var achievementCarousel = renderTopicAchievementMarketCarousel(data) ||
      renderTopicCoverflowCarousel(storyItems, { fallbackImage: "", title: "教学与创新成果" });
    ...
    var mediaLabel = topicMediaResourceLabel(data);
    ...
    if (mediaVideo) {
      ...
      videoHtml = '<div class="topic-showcase-video-box photo-frame" data-section-id="media" tabindex="0" role="button" aria-label="查看' + esc(mediaLabel) + '完整资料" data-lightbox="' + esc(videoPosterSrc) + '" data-caption="' + esc((videoPoster && videoPoster.caption) || data.name) + '">' +
      ...
      videoHtml = '<div class="topic-showcase-video-box topic-showcase-video-empty" data-section-id="media">' +
        '<span class="topic-showcase-coop-empty">' + (editMode ? "点击下方按钮上传视频（mp4 / webm）" : mediaLabel + "待上传") + '</span>' +
        (editMode ? '<button type="button" class="inline-edit-media-button inline-edit-control topic-showcase-video-upload" data-upload-video="media">上传视频</button>' : "") +
        '</div>';
    }
    return '<section class="topic-showcase-page" aria-label="' + esc(data.name) + '专题展示页" style="--topic-bg-image:url(\'/static/blueprint/' + esc(id) + '-bg.png\')">' +
      ...
      '<div class="topic-showcase-panel-head"><h3><span aria-hidden="true"></span>' + esc(mediaLabel) + '</h3>' +
      '<em data-video-more="1" role="button" tabindex="0" aria-label="查看全部' + esc(mediaLabel) + '">更多</em></div>' +
      videoHtml +
  ...
  function renderHome() {
    ...
    cards.forEach(function (cardConfig) {
      ...
      article.innerHTML =
        ...
        '<span class="fusion-label">' + esc(card.label || "系部专题") + '</span>' +
        '<h3>' + esc(t.name) + '</h3>' +
  ...
  function openDetail(kind, id, section, options) {
    ...
    if (!d) return;
    setPortalDocumentTitle("系部专题 · " + d.name);
  ...
  function renderExperienceLinks(links, compact, inert) {
    ...
    return '<div class="experience-links' + (compact ? " is-compact" : "") + '">' + list.map(function (link) {
      return '<a class="experience-link" href="' + esc(link.href) + '"' + (inert ? ' tabindex="-1"' : "") + '>' +
        '<span>' + esc(link.label || "打开特色系统") + '</span>' +
        '<em>打开</em>' +
        '</a>';
    }).join("") + '</div>';
  }
  ...
  function topicUsesSingleSystemsEntry(data) {
    return true;
  }

  function topicMediaResourceLabel(data) {
    return "数字资源";
  }

  function topicExperienceKind(link, data) {
    if (topicUsesSingleSystemsEntry(data)) return "systems";
  ...
  function renderTopicExperienceMenu(group, kind) {
    ...
    if (isResource && !group.length) return "";
    var label = isResource ? "数字资源" : "特色系统入口";
    var eyebrow = isResource ? "DIGITAL RESOURCES" : "FEATURED SYSTEMS";
    var icon = isResource
    ...
    var links = group.length ? group.map(function (link, index) {
      return '<a class="topic-experience-item experience-link" href="' + esc(link.href) + '" target="_blank" rel="noopener noreferrer">' +
        ...
        '<span class="topic-experience-copy"><strong>' + esc(link.label || "打开特色系统") + '</strong>' +
        '<small>' + esc(link.sourceTitle || (isResource ? "专题数字资源" : "专题特色系统")) + '</small></span>' +
    ...
    }).join("") : '<div class="topic-experience-empty">暂无特色系统入口，请在后台“特色系统入口”栏目中添加跳转按钮。</div>';
    return '<div class="topic-experience-action ' + (isResource ? "is-resource" : "is-system") + '" data-experience-kind="' + esc(kind) + '">' +
      ...
      (!isResource && INLINE_EDIT.enabled ? '<button class="topic-experience-edit inline-edit-control" type="button" data-systems-edit>编辑入口</button>' : '') +
  ...
  function currentSystemsLinks(data) {
    var section = systemsSectionFor(data);
    var sectionLinks = Array.isArray(section.externalLinks) ? section.externalLinks : [];
    var candidates = topicExternalLinks(data).filter(function (link) {
      return topicExperienceKind(link, data) === "systems";
    });
    return uniqueExternalLinks(sectionLinks.concat(candidates));
  }

  function systemsEditorOverlay() {
    if (!INLINE_EDIT.enabled) return;
    var data = currentDetailData();
    var project = detailProjectFor(data);
    if (!project) {
      inlineEditStatus("未关联后台项目，无法维护特色系统入口", "error");
      return;
    }
    var existing = document.querySelector(".systems-editor-overlay");
    if (existing) existing.remove();
    var item = systemsContentItemFor(data) || {};
    var links = currentSystemsLinks(data);
    if (!links.length) links = [{ label: "", href: "", sourceTitle: "特色系统入口" }];
    var overlay = document.createElement("div");
    overlay.className = "systems-editor-overlay";
    overlay.innerHTML =
      '<div class="systems-editor-panel" role="dialog" aria-label="维护特色系统入口">' +
      '<header><strong>维护特色系统入口</strong><button type="button" class="systems-editor-close" aria-label="关闭">×</button></header>' +
      '<p class="systems-editor-tip">每一行对应右侧按钮弹窗里的一个入口。可按合作商随时增删，或直接更换访问地址。</p>' +
      '<div class="systems-editor-list">' + links.map(function (link) {
        return '<div class="systems-editor-row">' +
          '<input class="systems-editor-label" value="' + esc(link.label || "") + '" placeholder="入口名称，如 智能制造实训系统">' +
          '<input class="systems-editor-url" value="' + esc(link.href || "") + '" placeholder="访问地址，如 https://...">' +
          '<button type="button" class="systems-editor-remove">删除</button>' +
          '</div>';
      }).join("") + '</div>' +
      '<button type="button" class="systems-editor-add">＋ 添加入口</button>' +
      '<div class="systems-editor-actions"><span class="systems-editor-status"></span>' +
      '<button type="button" class="systems-editor-save primary">保存入口</button></div>' +
      '</div>';
    document.body.appendChild(overlay);
    var statusNode = overlay.querySelector(".systems-editor-status");
    var setStatus = function (message, kind) {
      statusNode.textContent = message || "";
      statusNode.dataset.kind = kind || "";
    };
    var addRow = function (label, href) {
      var row = document.createElement("div");
      row.className = "systems-editor-row";
      row.innerHTML =
        '<input class="systems-editor-label" value="' + esc(label || "") + '" placeholder="入口名称，如 智能制造实训系统">' +
        '<input class="systems-editor-url" value="' + esc(href || "") + '" placeholder="访问地址，如 https://...">' +
        '<button type="button" class="systems-editor-remove">删除</button>';
      overlay.querySelector(".systems-editor-list").appendChild(row);
    };
    overlay.querySelector(".systems-editor-close").addEventListener("click", function () { overlay.remove(); });
    overlay.addEventListener("click", function (event) {
      if (event.target === overlay) overlay.remove();
      if (event.target.closest(".systems-editor-remove")) {
        event.preventDefault();
        var rows = overlay.querySelectorAll(".systems-editor-row");
        if (rows.length <= 1) {
          var row = event.target.closest(".systems-editor-row");
          row.querySelector(".systems-editor-label").value = "";
          row.querySelector(".systems-editor-url").value = "";
        } else {
          event.target.closest(".systems-editor-row").remove();
        }
      }
    });
    overlay.querySelector(".systems-editor-add").addEventListener("click", function () { addRow("", ""); });
    overlay.querySelector(".systems-editor-save").addEventListener("click", function () {
      var rows = Array.prototype.slice.call(overlay.querySelectorAll(".systems-editor-row"));
      var nextLinks = rows.map(function (row) {
        return {
          label: String(row.querySelector(".systems-editor-label").value || "").trim(),
          href: String(row.querySelector(".systems-editor-url").value || "").trim()
        };
      }).filter(function (link) { return link.label || link.href; });
      var invalid = nextLinks.find(function (link) { return !link.label || !link.href; });
      if (invalid) { setStatus("请把入口名称和访问地址填写完整", "error"); return; }
      setStatus("正在保存...", "warn");
      var title = "特色系统入口";
      var summary = nextLinks.length ? "维护当前专题对接的特色系统和合作商入口。" : "暂无特色系统入口。";
      var payload = {
        code: item.code || "EXT-" + String(data.id || "TOPIC").toUpperCase() + "-SYSTEMS",
        moduleKey: "systems",
        contentType: "article",
        title: title,
        subtitle: "",
        summary: summary,
        bodyJson: [{ type: "paragraph", text: summary }],
        metaJson: { "入口类型": "特色系统入口", "维护方式": "可视化编辑" },
        assets: nextLinks.map(function (link, index) {
          return {
            role: "external_link",
            caption: link.label,
            title: link.label,
      ...
      var url = "/api/projects/" + encodeURIComponent(project.id) + "/content-items" + (item.id ? "/" + encodeURIComponent(item.id) : "");
      inlineEditApi(url, {
        method: item.id ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      }).then(function () {
        return (item.extraItemIds || []).reduce(function (promise, id) {
          return promise.then(function () {
            return inlineEditApi("/api/projects/" + encodeURIComponent(project.id) + "/content-items/" + encodeURIComponent(id), { method: "DELETE" })
              .catch(function () { return null; });
          });
        }, Promise.resolve());
      }).then(function () {
        overlay.remove();
        inlineEditStatus("特色系统入口已保存", "ok");
        return reloadPortalDetail();
      }).catch(function (error) {
  ...
  function renderTopicExperienceActions(data) {
    ...
    var systems = links.filter(function (link) { return topicExperienceKind(link, data) === "systems"; });
    var resources = topicUsesSingleSystemsEntry(data) ? [] : links.filter(function (link) { return topicExperienceKind(link, data) === "resources"; });
  ...
  function renderTopicLoopPage(data) {
    ...
    return '<section class="topic-section-page topic-theme-' + esc(data.id) + '" style="--topic-section-bg:url(\'/static/blueprint/' + esc(data.id) + '-bg.png\')">' +
      ...
      '<div class="topic-section-heading"><span>' + esc(data.name || "系部专题") + ' · ' + indexLabel + '</span>' +
    ...
    return '<div class="topic-loop-media legacy-unused">' + list.map(function (b) {
      if (b.type === "video") {
        ...
        return '<figure class="topic-loop-video">' +
          ...
          '<figcaption>' + esc(b.title || "数字资源") + '</figcaption></figure>';
      }
  ...
  function renderTopicLoopMedia(mediaBlocks, data, section) {
    ...
    var items = list.map(function (b) {
      if (b.type === "video") {
        var poster = b.poster || "";
        return { title: b.title || displaySectionTitle(section), body: "", imageUrl: poster, caption: b.title || "数字资源", lightbox: "", video: true };
      }
  ...
  function renderTopicVideoList(data) {
    ...
    var listHtml = videos.length
      ...
      : '<li class="section-manager-empty">' + (editMode ? "点击“添加视频”上传视频和文字描述" : emptyLabel) + '</li>';
    return '<article class="topic-display-panel section-manager">' +
      '<div class="topic-display-meta"><p class="topic-display-category">' + esc(data.name || "系部专题") + '</p></div>' +
      '<h1 class="section-manager-title">' + esc(mediaLabel) + '</h1>' +
      '<div class="section-manager-items">' +
      '<div class="section-manager-items-head"><strong>' + esc(allLabel) + '</strong><em>' + videos.length + ' 个</em>' +
  ...
  function itemDetailOverlay(item, section, project) {
    ...
    if (editor && editMode) {
      ...
      overlay.querySelectorAll("[data-richtext-font], [data-richtext-size]").forEach(function (select) {
        select.addEventListener("change", function (e) {
          e.preventDefault();
          e.stopPropagation();
          editor.focus();
          if (select.dataset.richtextFont !== undefined && select.value) document.execCommand("fontName", false, select.value);
          if (select.dataset.richtextSize !== undefined && select.value) document.execCommand("fontSize", false, select.value);
          select.value = "";
        });
      });
  ...
  function renderTopicFullSection(section) {
    ...
    return '<article class="topic-display-panel content-type-' + esc(contentType) + ' section-manager">' +
      ...
      '<p class="topic-display-category">' + esc(data.name || "系部专题") + '</p>' +
  ...
  function groupSectionEntries(blocks, title, contentType) {
    ...
    (blocks || []).forEach(function (b) {
      ...
      if (b.type === "video" && (b.src || b.poster)) {
        var ve = { title: b.title || "数字资源", texts: [], images: [], videos: [b] };
        entries.push(ve);
        current = null;
        return;
      }
  ...
  function renderVideo(b) {
    return '<div class="video-player" data-vsrc="' + esc(b.src) + '" data-poster="' + esc(b.poster || "") + '">' +
      ...
      '<div class="v-info"><div class="v-title">' + esc(b.title || "数字资源") + '</div>' +
  ...
  function bindDelegates() {
    ...
    $("chapterPage").addEventListener("click", function (e) {
      var systemsEdit = e.target.closest("[data-systems-edit]");
      if (systemsEdit) {
        e.preventDefault();
        e.stopPropagation();
        systemsEditorOverlay();
        return;
      }
