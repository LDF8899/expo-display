...
def main():
    ...
    section_checks = {
        "frontend TOPIC_SECTION_ORDER/TITLES": parse_blueprint_topic_sections(),
        "backend TOPIC_MODULES": parse_server_topic_sections(),
    }
    for label, actual in section_checks.items():
        if actual != EXPECTED_TOPIC_SECTIONS:
            failures.append(f"{label} should be {EXPECTED_TOPIC_SECTIONS}, got {actual}")
